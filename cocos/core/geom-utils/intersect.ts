    ...
    return function capsule_capsule (capsuleA: capsule, capsuleB: capsule) {
        ...
        if (isDistAInBounds) {
            if (isDistBInBounds) {
                ...
                distance.pt_point_line2(v3_closestA, v3_closestB, startA, endA);
            }
        } else {
            if (isDistBInBounds) {
                ...
                distance.pt_point_line2(v3_closestB, v3_closestA, startB, endB);
            } else {
                // Neither of the distances along either line segment are within bounds.
                distA < 0 ? Vec3.copy(v3_closestA, startA) : Vec3.copy(v3_closestA, endA);
                distB < 0 ? Vec3.copy(v3_closestB, startB) : Vec3.copy(v3_closestB, endB);

                distance.pt_point_line2(v3_closestA_alt, v3_closestB, startA, endA);
                distance.pt_point_line2(v3_closestB_alt, v3_closestA, startB, endB);
